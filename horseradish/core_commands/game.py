...
import math
import re
from horseradish.config import cast_duration
...
def get_time_limit(connection):
    ...
    left = int(
        math.ceil((advance_call.when() - asyncio.get_running_loop().time()) / 60.0))
...
def switch(connection, player, team=None):
    ...
    if player.invisible:
        ...
        protocol.broadcast_chat('%s switched teams' % player.name)
...
def set_time_limit(connection, duration):
    ...
    protocol.broadcast_chat('Time limit set to {}'.format(span))
