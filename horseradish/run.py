...
def update_geoip(target_dir):
    from horseradish.config import MAXMIND_DOWNLOAD, MAXMIND_DOWNLOAD_MD5
...
def main():
    from horseradish.config import config, TOML_FORMAT, JSON_FORMAT
    ...
    if not sys.warnoptions:
        import warnings
        warnings.filterwarnings("default", module="horseradish[.*]")
    ...
    if args.version:
        import horseradish
        print("horseradish", horseradish.__version__)
        return
    ...
    # - ~/.config/horseradish/config.toml
    # - ~/.config/horseradish/config.json
    format_ = None
    ...
    with redirect_stdout(stdout), redirect_stderr(stderr):
        from horseradish import server
        server.run(loop)
