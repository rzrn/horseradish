...
.. note::
  ...
  This script depends on ``geoip2`` package and ``horseradish --update-geoip``
...
import os
from horseradish.commands import (command, restrict, get_player, target_player)
...
try:
    ...
    print(
        "('/from' command disabled due to missing GeoIP database. Run 'horseradish --update-geoip' to install.)"
    )
finally:
